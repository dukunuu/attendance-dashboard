      ...
      >
        {events
          ...
          .map((event) => {
            ...
            return isEditable ? (
              <EventContent key={event.id} />
            ) : (
              <Link
                href={`/lessons/${event.id}`}
                key={event.id}
        ...
        <div className="grid grid-cols-7 flex-1">
          {weekDates.map((hours, i) => {
            return (
              <div
                className={cn(
                  "h-full text-sm overflow-y-hidden text-muted-foreground border-l first:border-l-0",
...
const CalendarCurrentDate = () => {
  ...
  return (
    <time dateTime={date.toISOString()} className="tabular-nums">
      {format(date, view === "day" ? "dd MMMM yyyy" : "MMM yyyy", { locale })}
    </time>
  );
};
